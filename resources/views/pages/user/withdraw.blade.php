...
<div class="text-center" style="padding:50px">
                ...
                <div class="panel panel-default text-left" id="offerPanel" style="display:none">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="pull-right"><br><span style="color: red; font-weight: 700;">Примите обмен, и нажмите 'Проверить статус' что получить монетки на баланс.<span></span></span></div>
                </div>
                </div>
					<div class="panel panel-default text-left fw-6">
						<div class="panel-heading">
							<h3 class="panel-title"><b>Инвентарь : <span id="left_number">...</span> вещей</b></h3>
						</div>
						<div class="panel-body"><div class="btn-group" id="botFilter" style="margin-bottom:10px"></div>				       
					...
					<div class="panel panel-default text-left fw-4" style="vertical-align:top">
						...
						<div class="panel-body">
							<button class="btn btn-danger btn-lg" style="width:100%" onclick="showConfirm()" id="showConfirmButton">Выводимые предметы<div style="font-size:12px"><span id="sum">0</span> монеток | Баланс: <span id="bwithdraw">{{$u->money}}</span></div></button>				
        ...
        success: function (data) {
            ...
            var eleA = [];
                ...
                for(var i = 0; i < max; i++){
                	...
                	var slot_class = "";
                	var url = 'https://steamcommunity-a.akamaihd.net/economy/image/'+item.img+'/110fx50f';
					var DIV = "<div class='placeholder matched' data-name='"+item.market_hash_name+"' data-price='"+item.price+"' data-pos='"+predmet+"' data-hashname'"+item.market_hash_name+"'>";
    				DIV += "<div class='slot data-name='"+item.market_hash_name+"' data-pos='"+predmet+"' data-price='"+item.price+"' data-id='"+item.assetids+"' data-hashname='"+item.market_hash_name+"' style='background-image:url(\""+url+"\")'>";
    				DIV += "<div class='name'>"+item.market_hash_name+"</div>";
    				DIV += "<div class='price ball-0'>"+item.price+"</div>";
    				DIV += "</div></div>";                	
                    var ele = DIV;       
                    eleA.push(ele);
                    predmet++               
...
var socket = io.connect(':2020');
    socket
         .on('acceptTradeWithdraw', function (code,amount,tid,steamids) {
            if(steamid == steamids){
                $('#inlineAlert').html('');
                showPending(code,amount,tid);
            }
         })
         .on('declineTradeWithdraw', function(msg, steamids){
            if(steamid == steamids){
                $('#inlineAlert').html('');
